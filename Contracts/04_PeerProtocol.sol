...
contract PeerProtocol is ERC1155, Initializable, Ownable{
    ...
    TERC20 private token = TERC20(address(0x1BD7B233B054AD4D1FBb767eEa628f28fdE314c6)); //Polygon Testnet Token
    ...
    event Joined(address _from, address _to, uint _amount, uint timestamp, string method);
    ...
    // Function to allow users to participate in the loan by investing an amount.
    function joinLoan(uint amount) public {
        ...
        uint orinationFee = (originationNominal * ( 10000 + (loanRate*loanPeriod)/12))/tenthk;
        feePayable += orinationFee;
